<script lang="ts">
	...
	let sendOpen = $state(false); let invoiceRecipients = $state<string[]>([]); let invoiceMessage = $state('Please find the attached invoice.');
	...
	async function sendInvoice() { financialSaving = true; financialError = ''; try { const response = await fetch(`/admin/api/financials/${project.id}/send`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ recipientContactIds: invoiceRecipients, message: invoiceMessage }) }); const result = await response.json(); if (!response.ok) throw new Error(result.error); onfinancialchange('document', result.document); sendOpen = false; store.notify('Invoice sent'); } catch (error) { financialError = error instanceof Error ? error.message : 'Unable to send invoice.'; } finally { financialSaving = false; } }
...
{#if sendOpen}<div class="finance-backdrop"><form class="finance-modal" onsubmit={(event) => { event.preventDefault(); sendInvoice(); }}><h2>Send Invoice</h2><fieldset><legend>Recipients</legend>{#each emailContacts as contact}<label class="recipient"><input type="checkbox" checked={invoiceRecipients.includes(contact.id)} onchange={(event) => invoiceRecipients = event.currentTarget.checked ? [...invoiceRecipients, contact.id] : invoiceRecipients.filter((id) => id !== contact.id)} /> {contact.name || contact.email} — {contact.email}</label>{/each}</fieldset><label>Message<textarea rows="5" bind:value={invoiceMessage}></textarea></label>{#if financialError}<p class="email-error">{financialError}</p>{/if}<footer><button type="button" onclick={() => (sendOpen = false)}>Cancel</button><button class="primary" disabled={financialSaving}>Send Invoice</button></footer></form></div>{/if}
