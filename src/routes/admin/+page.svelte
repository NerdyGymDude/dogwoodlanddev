...
let emailSending=$state(false);
 ...
 const thread=$derived(store.mail.find(m=>m.id===selectedMail));
 const filteredActions=$derived(store.actions.filter(a=>actionFilter==='All'||a.state===actionFilter));
 const filteredMail=$derived(store.mail.filter(m=>mailbox==='All'||m.mailbox===mailbox));
 ...
 const searchResults=$derived(query.trim().length<2?[]:[...store.clients.filter(x=>`${x.name} ${x.email} ${x.phone}`.toLowerCase().includes(query.toLowerCase())).map(x=>({type:'Client',title:x.name,sub:x.email,id:x.id})),...store.projects.filter(x=>`${x.name} ${x.address} ${x.summary}`.toLowerCase().includes(query.toLowerCase())).map(x=>({type:'Project',title:x.name,sub:x.address,id:x.id})),...store.documents.filter(x=>x.name.toLowerCase().includes(query.toLowerCase())).map(x=>({type:'Document',title:x.name,sub:x.category,id:x.id})),...store.mail.filter(x=>`${x.sender} ${x.subject}`.toLowerCase().includes(query.toLowerCase())).map(x=>({type:'Email',title:x.subject,sub:x.sender,id:x.id}))]);
...
<div class="app">
 <aside class="sidebar">
  ...
  <nav>{#each nav as n}<button class:active={view===n[0]||(n[0]==='projects'&&view==='project')||(n[0]==='clients'&&view==='client')} onclick={()=>go(n[0])}><i>{n[2]}</i>{n[1]}{#if n[0]==='inbox'}<b>{store.mail.filter(m=>m.unread).length}</b>{/if}</button>{/each}</nav>
 ...
 <main class="main">
  ...
  <div class="content">
   ...
   {:else if view==='inbox'}
    <div class="page-heading compact"><div><p class="eyebrow">COMMUNICATIONS</p><h1>Inbox</h1></div><button class="primary" onclick={()=>modal='email'}>✎ Compose</button></div><div class="mailboxes">{#each ['All','Branch','Office','Accounting','Permitting'] as box}<button class:active={mailbox===box} onclick={()=>mailbox=box}><span>{box}</span><b>{box==='All'?store.mail.filter(m=>m.unread).length:store.mail.filter(m=>m.mailbox===box&&m.unread).length}</b></button>{/each}</div><div class="inbox"><div class="threads"><label>⌕ <input placeholder="Search mail"/></label>{#each filteredMail as m}<button class:active={selectedMail===m.id} class:unread={m.unread} onclick={()=>{selectedMail=m.id;store.markMailRead(m.id)}}><div><strong>{m.sender}</strong><time>{m.time}</time></div><h3>{m.subject}</h3><p>{m.preview}</p><span>{m.mailbox}</span></button>{/each}</div>{#if thread}<section class="message"><header><div><span class="status">{thread.mailbox}</span><h2>{thread.subject}</h2><p>From {thread.sender} &lt;{thread.email}&gt;</p></div><button>•••</button></header>{#if thread.projectId}<button class="context" onclick={()=>openProject(thread.projectId!)}><span>Linked project</span><strong>{store.projects.find(p=>p.id===thread.projectId)?.name}</strong><b>Open →</b></button>{/if}{#each thread.messages as msg}<article><div class="avatar">{msg.from.split(' ').map(x=>x[0]).join('')}</div><div><strong>{msg.from}</strong><time>{msg.time}</time><p>{msg.body}</p></div></article>{/each}<div class="reply"><textarea placeholder={`Reply from ${mailbox==='All'?thread.mailbox:mailbox.toLowerCase()}@dogwoodlanddev.com…`}></textarea><div><button onclick={()=>openQuickAdd('task')}>＋ Create task</button><button onclick={()=>store.setAction('a1','Waiting')}>Mark waiting</button><button class="primary" onclick={()=>store.notify('Reply queued in demo mail service')}>Send reply</button></div></div></section>{/if}</div>
   {:else if view==='tasks'}
